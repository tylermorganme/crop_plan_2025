...
console.log(`Backfill rowCover to existing plans`);
console.log(`Template: ${template.crops.length} crops`);
console.log(`With rowCover: ${template.crops.filter(c => c.rowCover).length}`);
...
function updateDatabase(dbPath, label) {
  ...
  try {
    ...
    for (const [configKey, config] of Object.entries(specs)) {
      ...
      // Add rowCover if template has it and config doesn't
      if (templateData.rowCover !== undefined && config.rowCover === undefined) {
        config.rowCover = templateData.rowCover;
        changed = true;
      }
